...
df_train_applprev_1 = scan_data_and_parse_dates(
    '../home-credit-credit-risk-model-stability/parquet_files/train/train_applprev_1_*.parquet')
df_train_applprev_2 = scan_data_and_parse_dates(
    '../home-credit-credit-risk-model-stability/parquet_files/train/train_applprev_2.parquet')
...
result_df = result_df.with_columns(
    result_df['credacc_cards_status_52L'].map_elements(lambda value: value[0] if isinstance(value, list) else value,
                                                       skip_nulls=False),
    result_df['cacccardblochreas_147M'].map_elements(lambda value: value[0] if isinstance(value, list) else value,
                                                     skip_nulls=False),
)
